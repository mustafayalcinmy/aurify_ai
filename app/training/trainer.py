    ...
    with tqdm(total=total_batches, desc="Toplam İlerleme", unit="iter") as pbar_total:
        for epoch in range(num_epochs):
            ...
            if (epoch + 1) % checkpoint_interval == 0:
                ...
                save_sequence_model(model, model_params, epoch+1, optimizer, avg_loss, checkpoint_path)
                
    # Son modeli kaydet
    final_path = os.path.join(save_dir, f"{model_name}_final.pt")
    save_sequence_model(model, model_params, num_epochs, optimizer, avg_loss, final_path)
